# sc-spatial-pipeline — how much of a spatial result is the pipeline, not the tissue?
...
> **Status: skeleton.** The stability machinery and its tests exist; no dataset processed.
...
**This repo measures the width of the tree.** Same dataset, same biological question, the
grid of reasonable preprocessing choices — and a direct answer to: *how many cells change
cell type, and does the spatial conclusion survive?*
...
So this is a sensitivity analysis, not a leaderboard. The output is not "method X wins"; it
is "the conclusion is stable across the choices that were never reported."
...
src/scspatial/
  ...
  report.py       the stability table and figures
```
...
`scanpy`, `squidpy` and `spatialdata` are optional extras imported lazily; the stability
machinery is pure Python and is what CI exercises.
